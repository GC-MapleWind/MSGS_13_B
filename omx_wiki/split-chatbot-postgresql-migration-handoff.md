...
- Current GHCR-hardening checkpoint comments are chatbot #1 `#issuecomment-4409887244` and main #55 `#issuecomment-4409887122`; documented wrapper evidence after GHCR first-tag parsing was fixed used main `dev` `27612f3caeb87b07737527659c545d146add7847` and chatbot `main` `8240db28ff058a216b017da1effb877d81290ee1`. Objective coverage passes, but live external gates still fail for chatbot workflow HTTP 404, no chatbot Actions runs, GHCR package HTTP 404, missing `workflow`/`read:packages` scopes, and both blocker issues OPEN. If the package later becomes visible, missing `latest`, full-sha, `main`, or `main-*` tags fail the checker, including when a required tag is first in the version tag list. Production/staging cutover evidence also requires `GC-MapleWind/maplewind-chatbot#1` to be `CLOSED` before the cutover starts.
