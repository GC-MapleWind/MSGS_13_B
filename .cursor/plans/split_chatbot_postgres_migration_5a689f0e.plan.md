...
- Main promotion completion-logic hardening: `check-external-gates.sh` and `prepare-main-promotion.sh` now fail directly on unpromoted `dev` commits, but do not require an open PR after `dev` has no unpromoted commits relative to `main`; `DEV_BRANCH=main PROD_BRANCH=main prepare-main-promotion.sh` exits `0`, proving the completed-promotion path is passable without a stale PR.
