...
- Remaining external gates and their evidence templates are indexed in `specs/001-split-chatbot-postgres/operator-handoff-index.md`; observable remote status can be checked with `specs/001-split-chatbot-postgres/check-external-gates.sh`. Gate A also has `specs/001-split-chatbot-postgres/prepare-chatbot-workflows.sh`, a guarded helper for workflow-scoped operators; it defaults to local preparation only and refuses push without workflow scope.
