...
- Main promotion checker hardening: `check-external-gates.sh` now includes a live `Main promotion gate`; latest direct readiness on the checker change exits `1` with 9 failures / 3 warnings because no open `dev` -> `main` promotion PR is visible and `dev` is still behind protected `main` by 2 commits, while `dev` is ahead by 179 unpromoted commits after recent handoff commits.
