...
- Documented handoff/evidence checkpoint: `origin/dev` `131a8cf5c7fdeab70cca062223b4b96b255b8f4a`; chatbot `main` remains `8240db28ff058a216b017da1effb877d81290ee1`.
- Readiness wrapper checkpoint on `131a8cf5c7fdeab70cca062223b4b96b255b8f4a` still exits `1`: objective coverage and parser self-tests pass, then live external gates report 6 failures / 2 warnings for missing `workflow`/`read:packages` scopes, absent chatbot `.github/workflows`, no chatbot Actions runs, GHCR package unreadable, and blocker issues #1/#55 still OPEN.
- Remote-tree proof is now documented in `completion-audit.md` and `operator-handoff-index.md`: a direct shallow clone of `GC-MapleWind/maplewind-chatbot` `main` at `8240db28ff058a216b017da1effb877d81290ee1` has no `.github/workflows` directory, corroborating the workflow blocker independently of the GitHub Contents API 404 response.
- Blocker checkpoint comments: chatbot #1 <https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410198610>; main #55 <https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410198609>. These are handoff/blocker checkpoints only, not completion evidence.
- Remaining external gates and their evidence templates are indexed in `specs/001-split-chatbot-postgres/operator-handoff-index.md`; observable remote status can be checked with `specs/001-split-chatbot-postgres/check-external-gates.sh`. Gate A also has `specs/001-split-chatbot-postgres/prepare-chatbot-workflows.sh`, a guarded helper for workflow-scoped operators; it defaults to local preparation only and refuses push without workflow scope. Helper executable-mode commit `b09c0f954a5efd39f038a45005d9c8fa10497a0f` passed dev Actions run `25583347519`. Gate B now has `specs/001-split-chatbot-postgres/validate-cutover-evidence-summary.sh` to preflight the final cutover evidence summary draft before posting to #55. Validator executable-mode commit `89976885674a311c1930419cb4ce1b101312cc33` passed dev Actions run `25583576116`.
- Documented external-gate check input after latest blocker checkpoint documentation was representative `origin/dev` checkpoint `816455254d22ab85f2404167943365e50770102a` and chatbot `origin/main` `8240db28ff058a216b017da1effb877d81290ee1`; `check-completion-readiness.sh` exited `1`: objective coverage passed, including task checkbox status (33 checked / 10 externally blocked unchecked), then `check-external-gates.sh` reported 7 failures / 2 warnings for chatbot `.github/workflows` HTTP 404, direct shallow-clone confirmation that `.github/workflows` is absent (checker commit `e60c6cfd21e53981ecfc9c077450513215edeb4b` passed dev Actions run `25584881776`), workflow files absent with no `workflow` scope, no chatbot Actions runs on `main` after timeout-retried API read (retry checker commit `0f04ad9448344238f4cf9073aff63fb3e6ca27c2` passed dev Actions run `25585084914`), GHCR package `GC-MapleWind/maplewind-chatbot` HTTP 404 after timeout-retried package read with no `read:packages` scope, and both blocker issues still OPEN. GHCR retry checker commit `b878846dd82bde86a301d0a9b5cd4d663fa20e3c` passed dev Actions run `25585243834` (`Build and Push Dev Image` 35s; `Deploy to Dev Server` 1m11s). If the package later becomes visible, missing `latest`, full-sha, `main`, or `main-*` tags fail the checker, including when a required tag is first in the version tag list. Issue-thread checkpoints are advisory handoff notes; latest advisory checkpoint comments are chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410749706` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410749794`. Later docs-only audit commits may advance `dev` without changing implementation or external gate state; rerun `check-completion-readiness.sh` for the current moving tip.
- Objective coverage guard `specs/001-split-chatbot-postgres/check-objective-coverage.py` passes after requiring the readiness wrapper file, covering 10 required objective files, all 43 task IDs, task checkbox status (33 checked / 10 externally blocked unchecked; guard commit `8b3ffbc748c3822c808c19a5a2d7a081fa6fd702` passed dev Actions run `25584668875`), all 16 FR IDs, all 7 SC IDs, 39 range-expanded prompt task IDs, 34 local Markdown links, and blocker language. This guard is documentation-only; it does not close external workflow/GHCR/cutover gates. The readiness wrapper now also verifies operator helper executability/syntax, the chatbot workflow helper commit-body self-test, and the cutover validator self-test before live external diagnostics. Readiness wrapper commit `4cf406dee9f7dbec73834b7cf42fa0c1f35b2bd9` passed dev Actions run `25583770806`. Helper/readiness checkpoint comments were posted to chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410494284` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410494343`. Gate A and Gate B live checking now require the full final evidence summary field sets before #1/#55 can satisfy readiness; Gate B checker commit `1b0728df8c84ca788af4c44112638ef66e8f787e` passed dev Actions run `25584011713`. Gate A checker commit `f6b3b81d66eacb438c639910f5cca80a3bada0e7` passed dev Actions run `25584235040` (`Build and Push Dev Image` 1m3s; `Deploy to Dev Server` 1m8s). Latest advisory issue checkpoints were posted to chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410749706` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410749794`; they do not close the gates.
...
- Dev workflow evidence checkpoint: run `25582935740` succeeded for checker commit `93f7bb3565978a0c27ab39849df64e842b1d92ab`, including `Build and Push Dev Image` and `Deploy to Dev Server`, after rejecting punctuated/spaced placeholder variants in required evidence fields. This remains main-dev evidence only. The checkpoint was echoed to chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410406164` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410406223`.
- Commit-message hygiene guard checkpoint: non-doc commit `03c7aaaabb665aba86dd4aaa843812dfd854a13d` added a non-blocking `check-completion-readiness.sh` advisory for already-pushed commit bodies that contain literal `\n`; dev Actions run `25587535583` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server`). Handoff sync commit `4852486f6b1ffcd08eaae19e756c53f429d17df5` recorded that guard in `completion-audit.md` and `operator-handoff-index.md`; it was Markdown-only, so no new Actions run was expected. Latest readiness on `4852486` still exited `1` with the same 7 external failures / 2 warnings plus the non-blocking hygiene advisory warning. Latest advisory comments remain chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410749706` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410749794`; they do not close the gates.
- Readiness wrapper executable checkpoint: non-doc commit `9792702fd1b13fe361b158f2b36dfe64440bcfb3` added the self executable-mode check and passed dev Actions run `25588002022`; follow-up commit `42734124602c3ae97e508f3d8ffe52b4f4ea5a05` stores the wrapper with git mode `100755` and passed dev Actions run `25588159350` (`Build and Push Dev Image`, `Deploy to Dev Server`). Direct wrapper invocation still exits `1` because the same live external Gate A/Gate B blockers remain.
- Main promotion preflight: no open PRs currently exist in `GC-MapleWind/MSGS_13_B`; `main` is protected at `51853f8284a0d0b30f891e45c37dfae55b0ab14b`; `main...dev` is diverged with `dev` ahead by 177 commits and behind by 2. This reinforces that dev workflow success is not production/cutover evidence.
- Main promotion checker hardening: commit `0cec9c8ee154bd979a757a3258d96af55ae8fb92` adds a live `Main promotion gate` to `check-external-gates.sh`; dev Actions run `25588518613` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server`). Latest direct readiness on `0cec9c8` exits `1` with 9 failures / 3 warnings because no open `dev` -> `main` promotion PR is visible and `dev` is still behind protected `main` by 2 commits, while `dev` still has unpromoted commits ahead of `main` after recent handoff commits. Later completion-logic hardening makes unpromoted `dev` commits a blocking failure until production promotion is complete, while allowing an already-promoted state to pass without requiring a stale open PR.
- Main reconciliation rehearsal: `origin/main` has two commits not in `origin/dev` (`c3fd0298c2baa5b1061c81fca688d387eff1391c` #52 and `51853f8284a0d0b30f891e45c37dfae55b0ab14b` #53). A non-committal `git merge --no-commit --no-ff origin/main` from current `origin/dev` completed cleanly with no worktree diff, but no promotion PR or merge was created because that remains an external release decision.
- Main promotion helper: commit `3dc31d75d1fc9a035087c0cb3753377e1c1024c4` adds `specs/001-split-chatbot-postgres/prepare-main-promotion.sh`, which repeats the non-destructive promotion dry-run and is wired into readiness helper self-tests; it exits non-zero while no visible promotion PR exists or `dev` remains behind protected `main`. Dev Actions run `25588828682` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server`).
- Promotion helper self-test hardening: commit `5b7e938e5cd044ad65696fd1fa31253be019f8b2` makes `prepare-main-promotion.sh --self-test` assert both a clean no-diff rehearsal and a dirty rehearsal that must be detected. Dev Actions run `25589034684` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server` with health check). Latest direct readiness still exits `1` with the helper self-test passing; after the promotion completion-logic hardening, live external gates report 10 failures / 2 warnings because unpromoted `dev` commits are now a direct failure rather than only a warning.
- Main promotion completion-logic hardening: commit `06cd8ec894462fc4f87ad5d5101dc44a330fba94` makes `check-external-gates.sh` and `prepare-main-promotion.sh` fail directly on unpromoted `dev` commits, but not require an open PR after `dev` has no unpromoted commits relative to `main`; `DEV_BRANCH=main PROD_BRANCH=main prepare-main-promotion.sh` exits `0`, proving the completed-promotion path is passable without a stale PR. Dev Actions run `25589426862` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server` with health check).
- Main promotion checker self-test coverage: commit `b8c9afa1910b4020c5bf5d7faed74dbb19654870` makes `check-external-gates.sh --self-test` cover the extracted promotion decision function for completed promotion without stale PR, unpromoted commits without PR, and unpromoted commits with PR. Dev Actions run `25589610767` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server` with health check).
- Main promotion helper decision self-test coverage: commit `55d2b8b9ccb93187bb6191c76725d86a25c65897` makes `prepare-main-promotion.sh --self-test` cover the same promotion decision cases, keeping the operator helper aligned with `check-external-gates.sh`. Dev Actions run `25589786448` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server` with health check).
...
- Recent `dev` evidence head before the T024/lifespan sync was `98419800cb139714dfe90aa9c28697484dc649ff`; dependency-cleanup evidence checkpoint is `7d70ea9ae35fc6ddc884b0af88dcf051bff20ff0`.
