...
- Latest external-gate check input after GHCR tag evidence was hardened was `origin/dev` `9b61f01dd8c8c80d86f44d48d32a4347937a849e` and chatbot `origin/main` `8240db28ff058a216b017da1effb877d81290ee1`; `check-completion-readiness.sh` exited `1`: objective coverage passed, then `check-external-gates.sh` reported 6 failures / 2 warnings for chatbot `.github/workflows` HTTP 404, workflow files absent with no `workflow` scope, no chatbot Actions runs on `main`, GHCR package `GC-MapleWind/maplewind-chatbot` HTTP 404 with no `read:packages` scope, and both blocker issues still OPEN. If the package later becomes visible, missing `latest`, full-sha, `main`, or `main-*` tags will now fail the checker. Latest blocker comments: chatbot #1 `#issuecomment-4409887244`, main #55 `#issuecomment-4409887122`. Later docs-only audit commits may advance `dev` without changing implementation state.
