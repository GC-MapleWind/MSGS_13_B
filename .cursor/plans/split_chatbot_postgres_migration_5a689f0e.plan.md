...
- Main promotion checker hardening: commit `0cec9c8ee154bd979a757a3258d96af55ae8fb92` adds a live `Main promotion gate` to `check-external-gates.sh`; dev Actions run `25588518613` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server`). Latest direct readiness on `0cec9c8` exits `1` with 9 failures / 3 warnings because no open `dev` -> `main` promotion PR is visible and `dev` is still behind protected `main` by 2 commits, while `dev` still has unpromoted commits ahead of `main` after recent handoff commits.
