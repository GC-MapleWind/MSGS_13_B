...
| T029/T030 chatbot CI/CD workflows | local branch `workflows-pending-scope` commit `6ab860c` contains `.github/workflows/ci.yml` and `deploy.yml`; direct push rejected by GitHub token missing `workflow` scope; patch preserved in `specs/001-split-chatbot-postgres/chatbot-workflows-pending.patch`; patch applies cleanly to current chatbot `origin/main` `6c76fbad89bfadaca4fe2eef5edaeca061e9640b` via `git apply --check` | GAP for remote workflow availability |
