...
| T022-T028 chatbot repo structure/env/Dockerfile/dev compose | `../maplewind-chatbot` file tree; runtime/history adoption is on remote `main`, now at `25ba79950d452fa07aadf486d253c4c7eb6f3b71`; `src/main.py` lifespan runs Alembic `upgrade head` by default, dev compose uses a named PostgreSQL volume, and local `docker compose -f docker-compose.dev.yml up --build -d` returned `/health` successfully even with dummy Google credentials | PASS |
