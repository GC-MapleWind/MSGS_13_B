...
| Required verification commands and gates from prompts/tasks | Evidence includes ruff, unittest, Alembic offline/online, Docker build, compose health, pgloader dry-run, local-link audit, `git diff --check`, and `check-external-gates.sh` outputs, including workflow, Actions, GHCR, and blocker-issue visibility; command-specific evidence is listed below | PASS for local/dev gates; external workflow and production gates remain open |
...
| T020 cutover runbook with rollback | `specs/001-split-chatbot-postgres/cutover-runbook.md`; `production-cutover-evidence-template.md` captures the required issue evidence fields for the ops run | PASS |
...
- External gate recheck on `origin/dev` `96af07881de00df7f70ae21aaab40656508d696f` and chatbot `origin/main` `8240db28ff058a216b017da1effb877d81290ee1`: `./specs/001-split-chatbot-postgres/check-external-gates.sh` exited `1` as expected while blocked; it confirmed chatbot `.github/workflows` still returns HTTP 404, current OAuth scopes are `gist, read:org, repo` without `workflow`, no chatbot Actions workflows are visible, GHCR package `GC-MapleWind/maplewind-chatbot` is not visible with the current credential, and blocker issues #1/#55 remain open. Latest issue checkpoint comments: chatbot #1 `#issuecomment-4409522721`, main #55 `#issuecomment-4409522898`.
...
4. Durable handoff evidence is published at https://github.com/GC-MapleWind/MSGS_13_B/blob/dev/omx_wiki/split-chatbot-postgresql-migration-handoff.md and cross-linked from both blocker issues. The remaining external gates are indexed in `specs/001-split-chatbot-postgres/operator-handoff-index.md`; `specs/001-split-chatbot-postgres/check-external-gates.sh` is a diagnostic status checker, not completion proof; it exits non-zero while observable blockers remain.
