...
4. Durable handoff evidence is published at https://github.com/GC-MapleWind/MSGS_13_B/blob/dev/omx_wiki/split-chatbot-postgresql-migration-handoff.md and cross-linked from both blocker issues. The remaining external gates are indexed in `specs/001-split-chatbot-postgres/operator-handoff-index.md`; `specs/001-split-chatbot-postgres/check-external-gates.sh` is a diagnostic status checker, not completion proof.
