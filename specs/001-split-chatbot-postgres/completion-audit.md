...
- Workflow patch local CI simulation against current chatbot `origin/main` `6c76fba` — PASS: `uv sync --dev --frozen`, `uv run ruff check .`, `CHATBOT_DATABASE_URL=sqlite+aiosqlite:///./ci-chatbot.db ... uv run python -m unittest discover -s tests` (6 tests), `uv run alembic upgrade head --sql`, `uv run alembic upgrade head` against temporary `postgres:17-alpine`, and `docker build -t chatbot-ci-local:workflow-patch .`.
