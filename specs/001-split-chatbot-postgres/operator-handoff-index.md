...
- Placeholder-evidence checker checkpoint: commit `93f7bb3565978a0c27ab39849df64e842b1d92ab` rejects punctuated/spaced placeholder variants in required evidence fields; dev Actions run `25582935740` succeeded. This is main-dev checker evidence only and does not close external gates. Issue checkpoints: chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410406164` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410406223`. Gate A helper executable-mode checkpoint `b09c0f954a5efd39f038a45005d9c8fa10497a0f` passed dev Actions run `25583347519`; it remains operator-prep evidence only. Gate B validator executable-mode checkpoint `89976885674a311c1930419cb4ce1b101312cc33` passed dev Actions run `25583576116`; it remains operator-prep evidence only. Readiness wrapper helper-check checkpoint `4cf406dee9f7dbec73834b7cf42fa0c1f35b2bd9` passed dev Actions run `25583770806`; it remains readiness-wrapper evidence only. Helper/readiness issue checkpoints: chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410494284` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410494343`.
- Commit-message hygiene advisory guard checkpoint: commit `03c7aaaabb665aba86dd4aaa843812dfd854a13d` adds a non-blocking readiness warning for already-pushed commit bodies that contain literal `\n` sequences; dev Actions run `25587535583` succeeded (`Build and Push Dev Image`, `Deploy to Dev Server`). This is history-hygiene visibility only and does not close workflow, GHCR, or cutover gates. Latest advisory comments: chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410749706` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410749794`.
...
Use `specs/001-split-chatbot-postgres/prepare-main-promotion.sh` to rerun that promotion dry-run. It is intentionally non-destructive: it fetches refs, reports compare status, checks for an open promotion PR only while unpromoted `dev` commits remain, runs a temporary no-commit merge rehearsal, and exits non-zero while the gate is still blocked. Once `dev` has no unpromoted commits relative to `main`, the helper no longer requires an open PR just to pass a completed promotion state. This completion-logic hardening is commit `06cd8ec894462fc4f87ad5d5101dc44a330fba94`; dev Actions run `25589426862` succeeded. The same decision path is covered by `check-external-gates.sh --self-test` as of `b8c9afa1910b4020c5bf5d7faed74dbb19654870`, including the completed-promotion/no-stale-PR case; dev Actions run `25589610767` succeeded.
The helper self-test was hardened in `5b7e938e5cd044ad65696fd1fa31253be019f8b2` and passed dev Actions run `25589034684`; `prepare-main-promotion.sh --self-test` now covers both a clean no-diff rehearsal and a dirty rehearsal that must be detected, plus the same promotion decision cases as the live checker as of `55d2b8b9ccb93187bb6191c76725d86a25c65897`; dev Actions run `25589786448` succeeded.
...
do not count. Full-field Gate B checker checkpoint `1b0728df8c84ca788af4c44112638ef66e8f787e` passed dev Actions run `25584011713`; it remains checker evidence only. Full-field Gate A checker checkpoint `f6b3b81d66eacb438c639910f5cca80a3bada0e7` passed dev Actions run `25584235040` (`Build and Push Dev Image` 1m3s; `Deploy to Dev Server` 1m8s); it remains checker/main-dev evidence only. Latest advisory checkpoints: chatbot #1 `https://github.com/GC-MapleWind/maplewind-chatbot/issues/1#issuecomment-4410749706` and main #55 `https://github.com/GC-MapleWind/MSGS_13_B/issues/55#issuecomment-4410749794`; these are not completion evidence. The objective coverage guard also enforces the current task checkbox ledger: 33 evidence-backed tasks checked and 10 external-gate tasks unchecked. Guard commit `8b3ffbc748c3822c808c19a5a2d7a081fa6fd702` passed dev Actions run `25584668875` (`Build and Push Dev Image` 39s; `Deploy to Dev Server` 1m10s). The live external gate checker now also direct-clones the chatbot remote and treats missing `.github/workflows` in that tree as an independent Gate A failure. Checker commit `e60c6cfd21e53981ecfc9c077450513215edeb4b` passed dev Actions run `25584881776` (`Build and Push Dev Image` 42s; `Deploy to Dev Server` 1m8s). Chatbot Actions API reads now retry transient timeout-class failures before reporting the live Actions gate. Retry checker commit `0f04ad9448344238f4cf9073aff63fb3e6ca27c2` passed dev Actions run `25585084914` (`Build and Push Dev Image` 51s; `Deploy to Dev Server` 1m8s). GHCR package/version API reads now also retry transient timeout-class failures before reporting package/tag blockers. GHCR retry checker commit `b878846dd82bde86a301d0a9b5cd4d663fa20e3c` passed dev Actions run `25585243834` (`Build and Push Dev Image` 35s; `Deploy to Dev Server` 1m11s).
