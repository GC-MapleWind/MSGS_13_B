...
The main repo release branch is also not promoted: latest preflight found no open PRs in `GC-MapleWind/MSGS_13_B`; `main` is protected at `51853f8284a0d0b30f891e45c37dfae55b0ab14b`; `main...dev` is diverged with `dev` ahead by 179 commits and behind by 2 after the latest handoff commits. `check-external-gates.sh` now checks this as a live `Main promotion gate`, so dev workflow success remains pre-production evidence only until operators reconcile and approve that promotion path.
